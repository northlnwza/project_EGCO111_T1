...
void logStatusChange(const char *device, const char *number, const char *status, const char *username) {
    FILE *logFile;
	char	path[100];
...
void updateTimeLog(const char *device, const char *number, long duration, const char *username) {
    FILE *timeLogFile;
	char	path[100];

	sprintf(path, "%s/%s/timelog.txt", USERS_DIR, username);
    timeLogFile = fopen(path, "a");
    if (timeLogFile) {
...
int usedevice(const char *username) {
    ...
    //createDevicesFile();
    numDevices = readDevices(devices, 100, username);
    ...
    while (1) {
        ...
        if (strcmp(statuses[index], "OFF") == 0) {
            ...
            if (startTimes[index] != 0) {
                ...
                updateTimeLog(devices[index * 2], devices[index * 2 + 1], duration, username);
                startTimes[index] = 0;
            }
        }

        logStatusChange(devices[index * 2], devices[index * 2 + 1], statuses[index], username);
    }
